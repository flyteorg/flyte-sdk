"""Example: Multi-input data join with primitive inputs (LLM approach).
...
- api_key parameter for LLM authentication
"""
...
@env.task
async def join_and_analyze(prompt: str, orders: pd.DataFrame, products: pd.DataFrame) -> tuple[File, File, float]:
    """Join two datasets, filter by threshold, and produce a report directory."""
    ...
    task = result.as_task(name="run_multi_input_join")
...
@env.task
async def multi_input_workflow(
...
Output a report_dir containing product_summary.csv (product_id, name, category,
total_orders, total_revenue) and overall_summary.csv (metric, value). Also output
grand_total as the total revenue across all filtered orders.""",
) -> tuple[File, File, float]:
    """Join two datasets and analyze: multiple data inputs, primitive args."""
    orders = build_orders()
    products = build_products()
    return await join_and_analyze(prompt=prompt, orders=orders, products=products)


if __name__ == "__main__":
    flyte.init_from_config()

    run = flyte.run(multi_input_workflow)
    print(f"\nRun URL: {run.url}")
