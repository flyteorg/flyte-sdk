...
def test_operator_tasks_registered_in_env(monkeypatch):
    ...
    assert any("step1" in name for name in env_task_names), f"step1 not found in env.tasks: {env_task_names}"
    assert any("step2" in name for name in env_task_names), f"step2 not found in env.tasks: {env_task_names}"
