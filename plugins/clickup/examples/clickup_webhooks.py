...
from flyteplugins.clickup import DEFAULT_SECRET_ENV, SAMPLE_DELIVERY, ClickUpProvider, events
...
async def launch_a_task(event):
    ...
    try:
        ...
        run = await idempotent_run.aio(task, key=event.dedupe_key(), task_id=event.resource_id)
