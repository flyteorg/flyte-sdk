"""MCP server builders for the ClickUp plugin.

`build_mcp_server` turns the plugin's tool registry into a FastMCP server, and
`clickup_mcp_app_env` wraps it in a Flyte `MCPAppEnvironment` that can be
...
DEFAULT_INSTRUCTIONS = (
    "ClickUp integration tools. Read tools fetch workspaces, spaces, lists, "
    "statuses, tasks, and comments. Write tools create or update tasks and "
    "add comments. Reacting to ClickUp webhooks is handled by the ClickUp "
    "app environment, not by this server."
)
...
def build_mcp_server(
    ...
    *,
    ...
    """Build a FastMCP server exposing the plugin's ClickUp tools.
    ...
    Returns:
        ...
        `clickup_mcp_app_env` to deploy it.
    """
    try:
        from mcp.server.fastmcp import FastMCP
        ...
        raise ModuleNotFoundError(
            "mcp is not installed. Install 'flyteplugins-clickup[mcp]' (or 'mcp') to build the MCP server."
        ) from exc
...
def clickup_mcp_app_env(
    name: str = "clickup-mcp",
    *,
    config: Config | None = None,
    token: str | None = None,
    read_only: bool = True,
    ...
    **app_kwargs: Any,
    ...
    """Create a Flyte `MCPAppEnvironment` serving the ClickUp MCP server.
    ...
    env = clickup_mcp_app_env("clickup-mcp")
    flyte.serve(env)
    ```
