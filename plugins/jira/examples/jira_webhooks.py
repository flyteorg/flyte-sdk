...
async def launch_a_task(event):
    ...
    live or has succeeded, rather than launching a second, so Jira redelivering an event — which
    ...
    result = await run_once.aio(task, key=event.dedupe_key(), issue_key=event.resource_id)
