...
from flyteplugins.jira import events
...
def test_constants_match_what_the_parser_produces():
    ...
    from flyteplugins.jira import JiraAppEnvironment, JiraEvent

    app = JiraAppEnvironment(name="events-test")

    event = JiraEvent(webhook_event="jira:issue_created")
    assert event.qualified_type == events.Issue.CREATED
    assert app._matches(events.Issue.CREATED, event)

    other = JiraEvent(webhook_event="comment_created")
    assert other.qualified_type == events.Comment.CREATED
...
def test_handlers_register_with_a_constant():
    ...
    @app.on_event(events.Issue.CREATED)
