...
from flyteplugins.jira._dispatch import (
    DUPE_LABEL_KEY,
    DuplicateRun,
    blocking_run,
    launch_task,
...
def test_run_name_for_is_legal():
    name = run_name_for("abc123def456" * 10, prefix="jr")
    assert len(name) <= 30
    assert name.isalnum()
    assert name.startswith("jr")
...
def test_blocking_run_finds_live_run():
    ...
    with (
        ...
        patch("flyteplugins.jira._dispatch._ensure_flyte_initialized"),
    ):
...
def test_launch_task_raises_on_duplicate():
    ...
    live.url = "http://run"
    with (
        patch("flyteplugins.jira._dispatch.blocking_run", return_value=live),
        patch("flyteplugins.jira._dispatch._ensure_flyte_initialized"),
    ):
...
def test_launch_task_launches_with_labels():
    ...
    with (
        patch("flyteplugins.jira._dispatch.blocking_run", return_value=None),
        patch("flyteplugins.jira._dispatch._ensure_flyte_initialized"),
        patch("flyteplugins.jira._dispatch._allocate_name", return_value="jrabc"),
    ...
    with_runcontext.assert_called_once_with(name="jrabc", labels={DUPE_LABEL_KEY: "k"})
    runner.run.assert_called_once_with(task, repo="octo/repo", number=1)


def test_allocate_name_skips_existing():
    from flyteplugins.jira._dispatch import _allocate_name

    with patch("flyteplugins.jira._dispatch._run_exists", side_effect=[True, False]):
        name = _allocate_name("jrabc")
    assert name == "jrabc1"
