...
the same event may be delivered more than once (Jira retries on non-2xx
...
async def launch_task(
    ...
    **Use `await launch_task.aio(...)` inside an async handler.** The synchronous
    ...
    loop that stalls every other in-flight request, and Jira times webhook
    ...
    Args:
        ...
        key: Stable dedupe key for the triggering event. `JiraEvent.dedupe_key()`
