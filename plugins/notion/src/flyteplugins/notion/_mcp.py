"""MCP server builders for the Notion plugin.

`build_mcp_server` turns the plugin's tool registry into a FastMCP server, and
`notion_mcp_app_env` wraps it in a Flyte `MCPAppEnvironment` that can be
...
DEFAULT_INSTRUCTIONS = (
    "Notion integration tools. Read tools search pages and databases and "
    "read their contents. Write tools create or update pages and append "
    "blocks. Detecting Notion changes (Notion has no webhooks) is handled "
    "by polling via the Notion app environment or a scheduled Flyte task, "
    "not by this server."
)
...
def build_mcp_server(
    ...
    *,
    ...
    """Build a FastMCP server exposing the plugin's Notion tools.
    ...
    Returns:
        ...
        `notion_mcp_app_env` to deploy it.
    """
    try:
        from mcp.server.fastmcp import FastMCP
        ...
        raise ModuleNotFoundError(
            "mcp is not installed. Install 'flyteplugins-notion[mcp]' (or 'mcp') to build the MCP server."
        ) from exc
...
def notion_mcp_app_env(
    ...
    **app_kwargs: Any,
    ...
    """Create a Flyte `MCPAppEnvironment` serving the Notion MCP server.
    ...
    env = notion_mcp_app_env("notion-mcp")
    flyte.serve(env)
    ```
