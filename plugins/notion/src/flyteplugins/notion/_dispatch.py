...
async def _ensure_flyte_initialized() -> None:
    ...
    The poll endpoint runs in an app process, not a task, so the SDK is
...
async def launch_task(
    ...
    **Use `await launch_task.aio(...)` inside an async handler.** The synchronous
    ...
    loop that stalls every other in-flight request, and a poll reporting many edited
    pages would launch them strictly one after another.

    Args:
        task: The task to launch — either a `flyte.remote.Task` looked up by
            name, or a local `TaskEnvironment` task object.
        key: Stable dedupe key for the triggering event. `NotionEvent.dedupe_key()`
