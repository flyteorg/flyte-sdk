"""Multi-agent research pipeline — Flyte orchestrating many Google ADK agents.

The "Flyte as the orchestrator harness" story for Google ADK. Instead of one agent,
a small team is composed with ordinary Flyte control flow, and each agent run is its
own durable, observable, cached Flyte action:
...
# A real deployment would call a search API here. Tools are Flyte tasks, so this could
# just as well be a GPU retrieval task, a warehouse query, or a browser session.
_CORPUS = {
