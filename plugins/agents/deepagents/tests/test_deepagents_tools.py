"""Unit tests for the deepagents adapter's tool bridge (no network / no controller)."""
...
from flyteplugins.agents.deepagents import tool


def test_task_becomes_langchain_tool_with_resolver():
    env = flyte.TaskEnvironment("da_tools_a")
    ...
    # The args schema is derived from the task's typed signature.
    assert "city" in get_weather.args_schema.model_fields


def test_create_deep_agent_accepts_the_tool():
    from deepagents import create_deep_agent

    from tests.test_deepagents_durable import _FakeInner

    env = flyte.TaskEnvironment("da_tools_b")
    ...
    agent = create_deep_agent(model=_FakeInner(), tools=[get_weather], system_prompt="x")
    assert hasattr(agent, "ainvoke")


@pytest.mark.asyncio
async def test_tool_dispatches_to_task_aio():
    env = flyte.TaskEnvironment("da_tools_c")
    ...
    with patch.object(multiply.task, "aio", new_callable=AsyncMock, return_value=42) as mock_aio:
        # Invoking the tool the way the agent does dispatches to task.aio().
...
def test_bare_and_parametrized_decorator_forms():
    env = flyte.TaskEnvironment("da_tools_d")
