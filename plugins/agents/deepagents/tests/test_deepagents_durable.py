"""Tests for deepagents model-turn durability (no network).
...
records/replays and that ``create_deep_agent`` accepts it and tool-calling flows.
"""
...
import flyteplugins.agents.deepagents._durable as durable_mod
...
@pytest.mark.asyncio
async def test_end_to_end_create_deep_agent_with_durable_model():
    """Build DurableChatModel over a fake inner, drive create_deep_agent, assert final text."""
    from deepagents import create_deep_agent

    inner = _FakeInner()
    dm = DurableChatModel(inner=inner)
    graph = create_deep_agent(model=dm, tools=[_weather_tool()], system_prompt="You are helpful.")
