"""Multi-agent research pipeline — Flyte orchestrating many Deep Agents.

This is the "Flyte as the orchestrator harness" story. Instead of one agent, a
small team of agents is composed with ordinary Flyte control flow, and each agent
run is its own durable, observable, cached Flyte action:

    plan --> research (parallel, one per subtopic) --> synthesize

- ``plan`` runs a planner agent that breaks the topic into subtopics.
- ``research`` runs a researcher deep agent per subtopic, fanned out in parallel
  (``asyncio.gather``); each is a separate durable child action with its own
  ``search_web`` tool calls, model turns, and report. Deep agents also plan
  internally (todos) and can spawn their own subagents — this composes: Flyte
  fans out the team, each member is a full deep agent.
- ``synthesize`` runs an editor agent that combines the findings.

Why this is more than a single agent with tools: every agent is a
first-class Flyte node — independently retried, cached, resource-sized, and
observable — and the fan-out is real distributed parallelism, not just asyncio in
one process.

Run:  flyte run deepagents_multi_agent.py research_pipeline --topic "The state of electric-vehicle batteries in 2025"
...
env = flyte.TaskEnvironment(
    ...
    image=flyte.Image.from_debian_base(name="deepagents-research").with_local_v2_plugins(
...
async def research(subtopic: str) -> str:
    """Researcher deep agent: investigate one subtopic using the search tool."""
