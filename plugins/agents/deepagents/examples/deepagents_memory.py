...
``run_agent(..., memory_key=...)`` backs the Deep Agent with a durable, keyed
``MemoryStore`` (object storage), so two runs with the same ``memory_key``
continue the same conversation — across workers and restarts. For deep agents
this covers *both* kinds of state: the conversation transcript AND the agent's
virtual filesystem (the files its built-in filesystem tools write), so notes it
takes in run 1 are still on disk in run 2.
...
Run:  flyte run deepagents_memory.py chat --message "Hi! My name is Alice and I love hiking." --memory_key user-alice
      flyte run deepagents_memory.py chat --message "What's my name and what do I like?" --memory_key user-alice
      (add `--local` after `run` to run locally; the shared `--memory_key` ties the two runs)
"""

import flyte

from flyteplugins.agents.deepagents import run_agent

env = flyte.TaskEnvironment(
    "deepagents-memory",
    resources=flyte.Resources(cpu=1),
    secrets=[flyte.Secret(key="anthropic_api_key", as_env_var="ANTHROPIC_API_KEY")],
    image=flyte.Image.from_debian_base(name="deepagents-memory").with_local_v2_plugins(
