"""Crash-and-resume: durable Deep Agent recovery on Flyte.

Shows that a crash mid-run does not redo completed work. On the first attempt
the deep agent does real work (model turns + tool calls), then the worker is
killed (simulated). Flyte retries the task; on the second attempt the completed
model turns replay from their ``flyte.trace`` records and the tool calls are
cache hits, so the agent finishes without re-calling (or re-billing) the model.
...
Run:  flyte run deepagents_crash_resume.py resilient_agent --question "What's the weather and population of Paris?"
...
env = flyte.TaskEnvironment(
    "deepagents-crash-resume",
    resources=flyte.Resources(cpu=1),
    secrets=[flyte.Secret(key="anthropic_api_key", as_env_var="ANTHROPIC_API_KEY")],
    image=flyte.Image.from_debian_base(name="deepagents-crash-resume").with_local_v2_plugins(
...
async def resilient_agent(question: str) -> str:
    ...
    answer = await run_agent.aio(
        ...
        model=_LoggingChatModel(inner=init_chat_model("anthropic:claude-sonnet-4-6")),
    )
