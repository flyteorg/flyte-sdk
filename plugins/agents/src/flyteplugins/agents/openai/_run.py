...
async def run_agent(
    ...
    *,
    ...
    Args:
        ...
        max_turns: Maximum model↔tool turns before the SDK raises.
    ...
    try:
        result = await Runner.run(agent, input, max_turns=max_turns, run_config=config)
    finally:
        if observability:
            await _flush_report()
