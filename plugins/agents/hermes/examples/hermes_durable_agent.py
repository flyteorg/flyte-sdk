...
Run:  flyte run hermes_durable_agent.py city_agent --question "What's the weather of Paris?"
      (or drive the sync variant:  flyte run hermes_durable_agent.py city_agent_sync --question "...")
      (add `--local` right after `run` to execute locally instead of on the backend)
"""

import flyte

from flyteplugins.agents.hermes import run_agent, run_agent_sync, tool

env = flyte.TaskEnvironment(
    "hermes-durable-agent",
    resources=flyte.Resources(cpu=1),
    secrets=[flyte.Secret(key="openai_api_key", as_env_var="OPENAI_API_KEY")],
    image=flyte.Image.from_debian_base(name="hermes-durable-agent").with_local_v2_plugins(
