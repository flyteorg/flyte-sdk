"""``FlyteAgent`` — build a Hermes agent with Flyte durability.

Users who want to construct their own Hermes ``Agent`` (custom prompts,
...
.. code-block:: python
    ...
    env = TaskEnvironment("hermes_agent_a")
    ...
    tools = agent.durable_tools(get_weather)  # durable Hermes tools
    built = agent.build(tools=tools)           # returns hermes.Agent
...
class FlyteAgent:
    """Build a Hermes ``Agent`` with Flyte durable tools, memory, and observability.
    ...
    Args:
        ...
        **kwargs: Additional kwargs forwarded to ``hermes.Agent.__init__``.
    ...
    def durable_tools(
        ...
        *tasks: typing.Any,
        ...
        """Convert Flyte tasks into durable Hermes tools.
        ...
        Returns:
            A list of Hermes-compatible tool callables ready to pass to
            ``Agent.run()``.
    ...
    def build(
        ...
        *,
        ...
        Args:
            tools: Durable Hermes tools (from :meth:`durable_tools`) or
                bare Flyte tasks. If ``None``, the agent is built without tools.
            agent: A pre-built Hermes ``Agent``. If provided, ``tools``
                is ignored and this agent is returned as-is.

        Returns:
            A Hermes ``Agent`` instance ready to call ``.run()`` on.
        ...
        if _HermesAgent is None:
            from hermes import Agent as _LocalAgent
        else:
            _LocalAgent = _HermesAgent
    ...
    async def run(
        ...
        **run_kwargs: typing.Any,
        ...
        """Build and run a Hermes agent in one call.
        ...
        Args:
            ...
            agent: A pre-built Hermes ``Agent``.
