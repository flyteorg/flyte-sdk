"""Cross-run Hermes memory — a thin handle over Flyte's keyed ``MemoryStore``.

Hermes keeps conversation state in-memory. This module provides the bridge:
it resolves a keyed ``MemoryStore`` and persists the thread's ``conversation_id``
(or equivalent) so a later run with the same ``memory_key`` continues the
conversation.
"""
...
_MEMORY_CONV_PATH = "hermes/conversation_id"
