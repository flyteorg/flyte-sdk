...
Hermes accepts tool callables as tools. :func:`tool` wraps a Flyte ``@env.task``
as a Hermes tool whose execution dispatches to the task via ``task.aio()`` — so
...
def tool(
    ...
    *,
    ...
    - For an ``@env.task``: returns a Hermes tool whose execution runs the task as
    ...
    - For a plain (async) callable: returns a Hermes tool that runs it inline.
...
def _task_to_tool(
    ...
    *,
    ...
    """Build a Hermes tool from a Flyte task."""
    ...
    # Hermes tools are typically plain async callables with a name/description.
...
def _callable_to_tool(
    ...
    *,
    ...
    """Build a Hermes tool from a plain callable."""
...
def _as_content(result: typing.Any) -> str:
    """Convert a tool result to a string for Hermes."""
...
def _coerce_tool(t: typing.Any) -> typing.Any:
    """Coerce a tool to a Hermes-compatible tool."""
