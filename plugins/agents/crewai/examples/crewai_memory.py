...
``run_agent(..., memory_key=...)`` backs the CrewAI session with a
...
Run:  flyte run crewai_memory.py chat --message "Hi! My name is Alice and I love hiking." --memory_key user-alice
      flyte run crewai_memory.py chat --message "What's my name and what do I like?" --memory_key user-alice
      (add `--local` after `run` to run locally; the shared `--memory_key` ties the two runs)
"""

import flyte

from flyteplugins.agents.crewai import run_agent

env = flyte.TaskEnvironment(
    "crewai-memory",
    resources=flyte.Resources(cpu=1),
    secrets=[flyte.Secret(key="openai_api_key", as_env_var="OPENAI_API_KEY")],
    image=flyte.Image.from_debian_base(name="crewai-memory").with_local_v2_plugins(
