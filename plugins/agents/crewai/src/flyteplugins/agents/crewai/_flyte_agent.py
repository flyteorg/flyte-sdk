"""``CrewAIAgent`` — build a CrewAI agent with Flyte durability.

Users who want to construct their own CrewAI ``Agent`` (custom prompts,
roles, goals, etc.) but still want durable tool execution,
cross-run memory, and observability can use :class:`CrewAIAgent`:
...
.. code-block:: python
    ...
    env = TaskEnvironment("crewai_agent_a")

    @env.task
    async def get_weather(city: str) -> str:
        return f"sunny in {city}"

    agent = CrewAIAgent("gpt-4o", name="weather-agent")
    tools = agent.durable_tools(get_weather)  # durable CrewAI tools
    built = agent.build(tools=tools)           # returns crewai.Agent
...
class CrewAIAgent:
    """Build a CrewAI ``Agent`` with Flyte durable tools, memory, and observability.
    ...
    Args:
        ...
        role: Agent role (e.g. "Assistant", "Researcher").
        goal: Agent goal/purpose.
        backstory: Agent backstory/context.
        system_prompt: Alternative to backstory for the agent's system prompt.
        **kwargs: Additional kwargs forwarded to ``crewai.Agent.__init__``.
    ...
    def __init__(
        ...
        *,
        ...
        role: str = "Assistant",
        goal: str = "Answer the user's question accurately and concisely.",
    ...
    def backstory(self) -> str:
        """The agent backstory."""
        if self._backstory is not None:
    ...
    def durable_tools(
        ...
        *tasks: typing.Any,
        ...
        """Convert Flyte tasks into durable CrewAI tools.
        ...
        Returns:
            A list of CrewAI-compatible tool callables ready to pass to
            ``Agent.run()``.
    ...
    def build(
        ...
        *,
        ...
        Args:
            tools: Durable CrewAI tools (from :meth:`durable_tools`) or
                bare Flyte tasks. If ``None``, the agent is built without tools.
            agent: A pre-built CrewAI ``Agent``. If provided, ``tools``
                is ignored and this agent is returned as-is.

        Returns:
            A CrewAI ``Agent`` instance ready to call ``.run()`` on.
    ...
    async def run(
        ...
        **run_kwargs: typing.Any,
        ...
        """Build and run a CrewAI agent in one call.
        ...
        Args:
            ...
            agent: A pre-built CrewAI ``Agent``.
        ...
        result = await built.run(input=input, tools=coerced_tools, **run_kwargs)
