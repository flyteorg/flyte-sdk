...
flyteplugins-agents-claude` is all the runtime image needs — no separate Node.js or
`@anthropic-ai/claude-code` install. (Older `claude-agent-sdk` releases that predate
bundling fall back to a `claude` on `PATH`.)
...
- [`claude_hitl.py`](examples/claude_hitl.py) — **human-in-the-loop**: a sensitive
  `issue_refund` tool pauses for a human to approve via the `flyteplugins-hitl` web
  form before it runs — a durable gate the agent SDK has no equivalent for.
