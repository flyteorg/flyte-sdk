...
- :class:`AgentExecutor` — build a LangChain ``AgentExecutor`` with durable tools,
  cross-run memory, and observability.
- :func:`tool` — turn a Flyte ``@env.task`` into a LangChain ``BaseTool`` that
  executes as a durable child action (own container/GPU, retries, caching).
- :func:`run_agent` — run the LangChain agent loop inside your task and return
  the final answer.
