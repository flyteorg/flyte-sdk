...
``@env.task`` as a LangChain ``StructuredTool`` whose ``arun`` dispatches to the
task via ``task.aio()`` — so when the agent calls the tool, it runs as a durable
Flyte child action (its own container/resources, with retries and caching) rather
than inline in the agent's process.
"""
...
def tool(
    ...
    *,
    ...
    - For an ``@env.task``: returns a ``StructuredTool`` whose ``arun`` runs the
      task as a durable Flyte child action when the agent invokes it. The input
...
def _task_to_tool(
    ...
    *,
    ...
    return LangChainTool(structured_tool=structured, _task=task, _name=tool_name)
