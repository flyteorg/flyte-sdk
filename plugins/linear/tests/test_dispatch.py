...
def test_run_name_for_is_legal():
    name = run_name_for("abc123def456" * 10, prefix="ln")
    assert len(name) <= 30
    assert name.isalnum()
    assert name.startswith("ln")
...
def test_launch_task_raises_on_duplicate():
    ...
    live.url = "http://run"
    with (
        patch("flyteplugins.linear._dispatch.blocking_run", return_value=live),
        patch("flyteplugins.linear._dispatch._ensure_flyte_initialized"),
    ):
...
def test_launch_task_launches_with_labels():
    ...
    with (
        patch("flyteplugins.linear._dispatch.blocking_run", return_value=None),
        patch("flyteplugins.linear._dispatch._ensure_flyte_initialized"),
        patch("flyteplugins.linear._dispatch._allocate_name", return_value="lnabc"),
    ...
    with_runcontext.assert_called_once_with(name="lnabc", labels={DUPE_LABEL_KEY: "k"})
    runner.run.assert_called_once_with(task, repo="octo/repo", number=1)


def test_allocate_name_skips_existing():
    from flyteplugins.linear._dispatch import _allocate_name

    with patch("flyteplugins.linear._dispatch._run_exists", side_effect=[True, False]):
        name = _allocate_name("lnabc")
    assert name == "lnabc1"
