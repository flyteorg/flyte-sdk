...
def test_constants_match_what_the_parser_produces():
    ...
    from flyteplugins.linear import LinearAppEnvironment, LinearEvent

    app = LinearAppEnvironment(name="events-test")

    event = LinearEvent(action="create", entity_type="Issue")
    assert event.qualified_type == events.Issue.CREATE
    assert app._matches(events.Issue.CREATE, event)

    other = LinearEvent(action="update", entity_type="Comment")
    assert other.qualified_type == events.Comment.UPDATE
...
def test_handlers_register_with_a_constant():
    ...
    @app.on_event(events.Issue.CREATE)
