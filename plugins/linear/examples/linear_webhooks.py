...
from flyteplugins.linear import SAMPLE_DELIVERY, LinearProvider, events
...
async def launch_a_task(event):
    ...
    live or has succeeded, rather than launching a second, so Linear redelivering an event — which
    ...
    result = await run_once.aio(task, key=event.dedupe_key(), issue_id=event.resource_id)
...
def _try_locally() -> None:
    ...
    secret = os.environ.setdefault(LinearProvider.default_secret_env, "local-trial-secret")
