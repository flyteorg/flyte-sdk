"""MCP server builders for the Slack plugin.

`build_mcp_server` turns the plugin's tool registry into a FastMCP server, and
`slack_mcp_app_env` wraps it in a Flyte `MCPAppEnvironment` that can be
...
DEFAULT_INSTRUCTIONS = (
    "Slack integration tools. Read tools fetch channels, messages, threads, "
    "and users. Write tools post or update messages, add reactions, and create "
    "channels. Reacting to Slack events is handled by the Slack app "
...
def build_mcp_server(
    ...
    *,
    ...
    """Build a FastMCP server exposing the plugin's Slack tools.

    Args:
        config: Plugin configuration; defaults to the module-level config.
        bot_token: Optional explicit bot token; otherwise read from the environment.
    ...
    Returns:
        ...
        `slack_mcp_app_env` to deploy it.
    """
    try:
        from mcp.server.fastmcp import FastMCP
        ...
        raise ModuleNotFoundError(
            "mcp is not installed. Install 'flyteplugins-slack[mcp]' (or 'mcp') to build the MCP server."
        ) from exc
...
def slack_mcp_app_env(
    ...
    **app_kwargs: Any,
    ...
    """Create a Flyte `MCPAppEnvironment` serving the Slack MCP server.
    ...
    env = slack_mcp_app_env("slack-mcp")
    flyte.serve(env)
    ```
    ...
    Args:
        ...
        bot_token: Optional explicit bot token (prefer mounting a secret instead).
        read_only: Only expose read tools (default True).
