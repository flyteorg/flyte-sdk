...
the same event may be delivered more than once (GitHub retries on non-2xx
...
async def launch_task(
    ...
    **Use `await launch_task.aio(...)` inside an async handler.** The synchronous
    ...
    loop that stalls every other in-flight request, and GitHub times webhook
    ...
    Args:
        ...
        key: Stable dedupe key for the triggering event. `GitHubEvent.dedupe_key()`
