...
from flyteplugins.github._dispatch import (
    DUPE_LABEL_KEY,
    DuplicateRun,
    blocking_run,
    launch_task,
...
def test_run_name_for_is_legal():
    name = run_name_for("abc123def456" * 10, prefix="gh")
    assert len(name) <= 30
    assert name.isalnum()
...
def test_blocking_run_finds_live_run():
    ...
    with (
        ...
        patch("flyteplugins.github._dispatch._ensure_flyte_initialized"),
    ):
...
def test_launch_task_raises_on_duplicate():
    ...
    live.url = "http://run"
    with (
        patch("flyteplugins.github._dispatch.blocking_run", return_value=live),
        patch("flyteplugins.github._dispatch._ensure_flyte_initialized"),
    ):
...
def test_launch_task_launches_with_labels():
    ...
    with (
        patch("flyteplugins.github._dispatch.blocking_run", return_value=None),
        patch("flyteplugins.github._dispatch._ensure_flyte_initialized"),
        patch("flyteplugins.github._dispatch._allocate_name", return_value="ghabc"),
    ...
    with_runcontext.assert_called_once_with(name="ghabc", labels={DUPE_LABEL_KEY: "k"})
    runner.run.assert_called_once_with(task, repo="octo/repo", number=1)


def test_allocate_name_skips_existing():
    from flyteplugins.github._dispatch import _allocate_name

    with patch("flyteplugins.github._dispatch._run_exists", side_effect=[True, False]):
        name = _allocate_name("ghabc")
    assert name == "ghabc1"
