...
# Import every scenario task module once, up front. Two reasons: their module-level
# TaskEnvironment()s register before the first flyte.init so a later import can't
# reset routing; and — critically for `app` — this pulls each task module into the
# fast-register code bundle, so the app-serving pod can re-import it. A lazily
# imported module is absent from the bundle and fails to import when served.
# Importing `app` builds a FastAPI app at import time; fastapi is a suite dependency
# (declared in the dev group), so it's available wherever this suite is collected.
