...
async def test_structured_dataset_in_dataclass(ctx_with_test_raw_data_path):
    ...
    sd = DataFrame.create(val=df, file_format="parquet")
...
async def test_structured_dataset_collection(ctx_with_test_raw_data_path):
    ...
    await TypeEngine.to_literal(DataFrame.from_val(df), WineType, TypeEngine.to_literal_type(WineType))
    ...
    lv = await TypeEngine.to_literal([[DataFrame.from_val(df)]], WineTypeListList, lt)
    assert lv is not None
