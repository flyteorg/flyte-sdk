...
def test_run_mode_var_default_is_none():
...
def test_run_mode_var_can_be_set_to_local():
...
def test_run_mode_var_can_be_set_to_remote():
...
def test_run_mode_var_can_be_set_to_hybrid():
...
def test_get_main_run_mode_returns_none_by_default():
    """Test that _get_main_run_mode returns None when not set."""
    _run_mode_var.set(None)
    assert _get_main_run_mode() is None


def test_get_main_run_mode_returns_local():
...
def test_get_main_run_mode_returns_remote():
...
def test_flyte_run_local_mode():
    ...
    sample_env = flyte.TaskEnvironment(name="test-run-mode")
...
@pytest.mark.asyncio
async def test_flyte_run_aio_local_mode():
    """Test that flyte.run.aio in local mode works correctly."""
    await flyte.init.aio()
    _run_mode_var.set(None)  # Reset

    sample_env = flyte.TaskEnvironment(name="test-run-mode-aio")
...
def test_file_from_local_in_local_mode():
    """Test that File.from_local works in local mode without uploading."""
    from flyte.io import File

    flyte.init()

    sample_env = flyte.TaskEnvironment(name="test-run-mode-file")
...
@pytest.mark.asyncio
async def test_file_from_local_aio_in_local_mode():
    """Test that File.from_local (async) works in local mode without uploading."""
    from flyte.io import File

    await flyte.init.aio()

    sample_env = flyte.TaskEnvironment(name="test-run-mode-file-aio")
...
def test_dir_from_local_in_local_mode():
    """Test that Dir.from_local works in local mode without uploading."""
    from flyte.io import Dir

    flyte.init()

    sample_env = flyte.TaskEnvironment(name="test-run-mode-dir")
...
@pytest.mark.asyncio
async def test_dir_from_local_aio_in_local_mode():
    """Test that Dir.from_local (async) works in local mode without uploading."""
    from flyte.io import Dir

    await flyte.init.aio()

    sample_env = flyte.TaskEnvironment(name="test-run-mode-dir-aio")
...
def test_dataframe_from_local_in_local_mode():
    ...
    sample_env = flyte.TaskEnvironment(name="test-run-mode-dataframe")
...
@pytest.mark.asyncio
async def test_dataframe_from_local_aio_in_local_mode():
    ...
    sample_env = flyte.TaskEnvironment(name="test-run-mode-dataframe-aio")
