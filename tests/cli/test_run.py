    ...
    def test_run_with_local_file_input(self, runner):
        ...
        try:
            cmd = [
                ...
                "--e", "[1,2,3]",
                "--f", '{"x":1.0, "y":2.0}',
                "--g", str(PARQUET_FILE),  # File input - uses local path in local mode
                "--i", "2020-05-01",
                "--j", "P1D",
                "--k", "RED",
                "--h",
                "--m", '{"hello": "world"}',
                "--p", "Any",
                "--q", str(RUN_TESTDATA),  # Dir input
                "--r", json.dumps([{"i": 1, "a": ["h", "e"]}]),
                "--s", json.dumps({"x": {"i": 1, "a": ["h", "e"]}}),
                "--t", json.dumps({"i": [{"i": 1, "a": ["h", "e"]}]}),
            ]
            result = runner.invoke(run, cmd)
            assert result.exit_code == 0, result.output
        except ValueError as ve:
    ...
    def test_run_with_local_dir_input(self, runner):
        ...
        try:
            cmd = [
                ...
                "--m", '{"hello": "world"}',
                "--p", "Any",
                "--q", str(RUN_TESTDATA),  # Dir input - uses local path in local mode
                "--r", json.dumps([{"i": 1, "a": ["h", "e"]}]),
                "--s", json.dumps({"x": {"i": 1, "a": ["h", "e"]}}),
                "--t", json.dumps({"i": [{"i": 1, "a": ["h", "e"]}]}),
            ]
