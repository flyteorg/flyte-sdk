...
def fnmatch(name, pat):
    """Test whether FILENAME matches PATTERN (case-insensitive)."""
    name = name.lower()
    pat = pat.lower()

    try:
        re_pat = _fnmatch_cache[pat]
    except KeyError:
        res = _fnmatch_translate(pat)
        if len(_fnmatch_cache) >= _FNMATCH_MAXCACHE:
            _fnmatch_cache.clear()
        _fnmatch_cache[pat] = re_pat = re.compile(res)
...
class Pattern:
    ...
    def normalize(cls, p):
        """
        Normalize a pattern by removing trailing spaces, leading/trailing
        slashes, and resolving . and .. components.
        """
    ...
    def match(self, filepath):
        """Check if filepath matches this pattern."""
...
class PatternMatcher:
    """
    Matches file paths against .dockerignore-style patterns.
    ...
    Usage:
        ...
        # Walk a directory
        for path in pm.walk('/path/to/root'):
            print(f"Include: {path}")

        # Check individual file
        if not pm.matches('test.pyc'):
            print("test.pyc is included")
    """

    def __init__(self, patterns):
        """
        Initialize with a list of .dockerignore-style patterns.

        Args:
            patterns: List of pattern strings. Patterns starting with '!'
                     are negations (explicit inclusions).
        """
    ...
    def walk(self, root):
        """
        Walk a directory tree, yielding paths that are not excluded.

        Args:
            root: Root directory path to walk

        Yields:
            Relative paths (strings) that should be included
        """
        ...
        def rec_walk(current_dir):
            for f in os.listdir(current_dir):
                ...
                if match:
                    # If we want to skip this directory, first check if there's
                    # an exclusion pattern (e.g. !dir/file) that starts with this
                    # dir. If so, we can't skip this dir.
                    skip = True
