...
- **Memory**: optional :class:`AgentMemory` that serializes conversation history
  and named artifacts to / from :class:`flyte.io.File` or :class:`flyte.io.Dir`,
  letting the agent persist state across runs (e.g. across scheduled wake-ups
  or webhook invocations).
...
# ----------------------------------------------------------------------------
# Memory (File / Dir backed)
# ----------------------------------------------------------------------------


@dataclass
class AgentMemory:
    """Conversation + artifact memory backed by :class:`flyte.io.File` / :class:`Dir`.

    Use this to give a long-running agent persistence across wake-ups:

    - Append every message to ``messages`` (the live conversation transcript).
    - Stash anything else (vector store refs, scratchpads, JSON blobs) by name
      in ``artifacts``.
    - Call :meth:`save_to_file` / :meth:`save_to_dir` at the end of a run, and
      :meth:`load_from_file` / :meth:`load_from_dir` at the start of the next.
    """
    ...
    def to_json(self) -> str:
        return json.dumps({"messages": self.messages, "artifacts": self.artifacts}, default=str)

    @classmethod
    def from_json(cls, data: str) -> "AgentMemory":
    ...
    async def save_to_file(self, file: File | None = None) -> File:
        """Serialize this memory as a single JSON file in remote storage.

        Pass an existing :class:`flyte.io.File` to overwrite that path; omit it to
        write to a fresh remote path generated by :meth:`File.new_remote`.
        """
        target = file or File.new_remote(file_name="agent_memory.json")
        payload = self.to_json().encode("utf-8")
        async with target.open("wb") as f:
            await f.write(payload)  # type: ignore[union-attr]
        return target

    @classmethod
    async def load_from_file(cls, file: File) -> "AgentMemory":
        """Restore memory previously written by :meth:`save_to_file`."""
        async with file.open("rb") as f:
            data = bytes(await f.read()).decode("utf-8")  # type: ignore[union-attr]
        return cls.from_json(data)

    async def save_to_dir(self, remote_destination: str | None = None) -> Dir:
        """Serialize memory into a directory with messages + each artifact in its own file.

        Useful when artifacts are large enough that they should be addressable
        independently, or when downstream tools want to consume artifacts as
        files.

        The layout is::

            <dir>/messages.json
            <dir>/artifacts/<key>.json

        Strings are written verbatim; everything else is JSON-encoded.
        """
        import tempfile

        import aiofiles

        local_root = tempfile.mkdtemp(prefix="flyte_agent_mem_")
        messages_path = os.path.join(local_root, "messages.json")
        async with aiofiles.open(messages_path, "w", encoding="utf-8") as f:
            await f.write(json.dumps(self.messages, default=str))

        artifacts_dir = os.path.join(local_root, "artifacts")
        if self.artifacts:
            os.makedirs(artifacts_dir, exist_ok=True)
            for key, value in self.artifacts.items():
                target = os.path.join(artifacts_dir, f"{_safe_filename(key)}.json")
                content = value if isinstance(value, str) else json.dumps(value, default=str)
                async with aiofiles.open(target, "w", encoding="utf-8") as f:
                    await f.write(content)
        return await Dir.from_local(local_root, remote_destination=remote_destination)

    @classmethod
    async def load_from_dir(cls, dir: Dir) -> "AgentMemory":
        """Restore memory previously written by :meth:`save_to_dir`.

        Falls back gracefully when individual files are missing.
        """
        memory = cls()
        async for file in dir.walk(recursive=True):
            name = file.name or os.path.basename(file.path)
            relpath = file.path
            try:
                async with file.open("rb") as f:
                    raw = bytes(await f.read()).decode("utf-8")  # type: ignore[union-attr]
            except Exception:
                logger.debug("AgentMemory.load_from_dir: unable to read %s", relpath, exc_info=True)
                continue

            if name == "messages.json":
                try:
                    memory.messages = json.loads(raw)
                except json.JSONDecodeError:
                    logger.warning("messages.json was not valid JSON; ignoring.")
                continue

            if "/artifacts/" not in relpath and not relpath.endswith(name):
                continue
            if name.endswith(".json"):
                ...
                key = name
            try:
                memory.artifacts[key] = json.loads(raw)
            except json.JSONDecodeError:
                memory.artifacts[key] = raw
        return memory
...
class Agent:
    ...
    memory:
        Optional :class:`AgentMemory` initialized from a previous session. When
        provided, the existing transcript is prepended to every conversation
        and the in-flight transcript is appended to it on each call.
    approval_callback:
