...
class WebhookAppEnvironment(FastAPIAppEnvironment):
    ...
    # Defaulted to None and built in __post_init__, unlike the base class which
    # requires an app up front — callers configure providers, not FastAPI.
    ...
    def __post_init__(self):
        if not self.providers:
            raise ValueError(
                ...
                "`from flyteplugins.github import GitHubProvider` then `providers=[GitHubProvider()]`"
            )
        bad = [p for p in self.providers if not isinstance(p, Provider)]
        if bad:
            raise TypeError(f"providers must be Provider instances, got {bad!r}")
    ...
    async def _handle(self, provider_name: str, request: Any) -> Any:
        ...
        if provider is None:
            configured = sorted(self._by_name)
            raise HTTPException(
                status_code=404,
                detail=f"provider {provider_name!r} is not configured on this app; configured: {configured}",
            )

        body = await request.body()
        headers = dict(request.headers)

        # Setup handshakes (GitHub's ping, Slack's url_verification) are answered
        # before verification, since the product sends them to prove reachability
        # before any secret is in play. Providers that have none skip this.
        if provider.handshake is not None:
            answer = provider.handshake(headers, body)
