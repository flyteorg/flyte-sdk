...
from flyte._internal.runtime.io import _INPUTS_FILE_NAME
...
def get_task_inputs(task_module_name, task_name, context_working_dir):
    ...
    """
    local_inputs_file = os.path.join(context_working_dir, _INPUTS_FILE_NAME)
    idl_input_literals = load_proto_from_file(_literals_pb2.LiteralMap, local_inputs_file)
...
def run_async_execute_command(cmd: str):
    asyncio.run(asyncio.run(execute_command(cmd)))
