...
claude_code_task = ContainerTask(
    ...
    outputs={"generated_code": Dir, "exit_code": str, "log": str},
    command=[
        "/bin/bash",
        "-c",
        (
...
async def create_working_directory() -> Dir:
    ...
    (temp_dir / "README.md").write_text("# Claude Code Generated Project\n\nThis project was generated by Claude Code.\n")
...
async def claude_code_generator(
    ...
    try:
        ...
        return working_dir, f"Error: {str(e)}", "1", False


@env.task 
async def main(
    prompt: str = "Create a simple Python web API using FastAPI that has endpoints for creating, reading, updating and deleting users. Include proper error handling and validation.",
    max_turns: int = 10,
    budget_limit: float = 5.0,
    timeout_minutes: int = 30
) -> str:
