...
give the sidecar a GPU + CUDA image + layer offload. All `flyteplugins.*` imports are lazy so the
client task container -- flyte + openai only -- never imports them.
...
def _volume_name(artifact_name: str, artifact_version: str) -> str:
    ...
    A digest of the full name@version keeps it unique and bounded even when ARTIFACT_NAME is long
    or unusual; the sanitized name is a best-effort readable prefix.
    """
    import hashlib
    import re
...
def _volume_sidecar(serve_image_uri: str, locator: str, serve_cmd: list[str]) -> flyte.PodTemplate:
    ...
    # GPU goes on the SIDECAR: nvidia.com/gpu needs request==limit + a GPU toleration.
    tolerations = None
    if GPU:
...
if __name__ == "__main__":
    ...
    pod_template = _volume_sidecar(built.uri, locator, serve_cmd)
