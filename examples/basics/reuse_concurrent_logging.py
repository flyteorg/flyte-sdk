...
Workers exercise four logger configs to confirm context stamping works
regardless of how the user obtains a logger:
  ...
  4. register_log_context contextvar        — stamps a custom field on every record
"""
...
import flyte
from flyte._logging import register_log_context
...
# ContextFormatter prints the [run][action] prefix even though this logger
# lives outside the flyte.* namespace.
...
# Variant 4: register a custom contextvar that gets stamped on every LogRecord
# as record.batch_id. JSON output would include it as a top-level field; a
# console formatter can reference it via %(batch_id)s.
batch_id_var: contextvars.ContextVar[str | None] = contextvars.ContextVar("batch_id", default=None)
register_log_context("batch_id", batch_id_var)
...
@env.task
async def worker_with_context_var(label: str, batch_id: str, ticks: int = 3) -> str:
    """Variant 4: custom contextvar stamped on every record via register_log_context."""
    token = batch_id_var.set(batch_id)
    try:
        flyte.logger.info("starting label=%s batch_id=%s", label, batch_id)
        for i in range(ticks):
            await asyncio.sleep(1)
            flyte.logger.info("label=%s tick=%d/%d", label, i + 1, ticks)
        flyte.logger.info("done label=%s", label)
...
async def main(n: int = 2) -> list[str]:
    """
    Dispatches a mix of all four worker variants so logs from each logger
    ...
    for i in range(n):
        ...
        coros.append(worker_with_context_var(label=f"ctxvar-{i}", batch_id=f"batch-{i}"))
